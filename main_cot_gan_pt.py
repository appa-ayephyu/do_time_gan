...
def train(args):
    ...
    with urllib.request.urlopen(
        "https://konstantinklemmer.github.io/data/data_stml/lgcp_64_1000_gneiting.json"
    ) as url:
        data = np.array(json.loads(url.read().decode())).astype(np.float32)
    data = torch.tensor(data)
    x_height, x_width, total_time_steps = data.shape

    data_seq = torch.reshape(
        data, (x_height, x_width, total_time_steps // time_steps, time_steps)
    )
    raw_data = (
        data_seq.permute(0, 2, 1, 3)
        .permute(1, 0, 2, 3)
        .permute(0, 1, 3, 2)
        .permute(0, 2, 1, 3)
    )
    # normalise data between 0 and 1
    data = (raw_data - torch.min(raw_data)) / (
        torch.max(raw_data) - torch.min(raw_data)
    )
    ...
    with open("./trained/{}/train_notes.txt".format(saved_file), "w") as f:
        ...
        f.write("MODEL_DATA: {}\nSEQ_LEN: {}\n".format(test_, time_steps))
        f.write("STATE_SIZE: {}\nLAMBDA: {}\n".format(g_state_size, reg_penalty))
        f.write(
            "BATCH_SIZE: {}\nCRITIC_ITERS: {}\nGenerator LR: {}\n".format(
                batch_size, gen_lr, disc_lr
            )
        )
        f.write("SINKHORN EPS: {}\nSINKHORN L: {}\n\n".format(sinkhorn_eps, sinkhorn_l))
    ...
    optimizerD = optim.Adam(
        params=(
            list(discriminator_h.parameters()) + list(discriminator_m.parameters())
        ),
        lr=disc_lr,
        betas=(beta1, beta2),
    )
    # optimizerDH = optim.Adam(
    #     discriminator_h.parameters(), lr=disc_lr, betas=(beta1, beta2)
    # )
    # optimizerDM = optim.Adam(
    #     discriminator_m.parameters(), lr=disc_lr, betas=(beta1, beta2)
    # )
    ...
    for _ in range(epochs):
        for x in loader:
            ...
            loss_d = gan_utils.compute_mixed_sinkhorn_loss(
                real_data,
                fake_data,
                m_real,
                m_fake,
            ...
            gen_loss = gan_utils.compute_mixed_sinkhorn_loss(
                real_data,
                fake_data,
                m_real,
                m_fake,
            ...
            if torch.isinf(gen_loss):
                ...
                if it_counts % save_freq == 0 or it_counts == 1:
                    ...
                    img = samples.reshape(
                        1, batch_size * x_height, time_steps * x_width
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dname", type=str, default="spatial_data", choices=["spatial_data"]
    )
    ...
    parser.add_argument("-p", "--path", type=str, default="/home/")
