...
from time_gan_pt.models.utils import (
    timegan_trainer,
    timegan_generator,
    do_timegan_trainer,
    ex_inference,
...
def main(args):
    ...
    # Runtime directory
    code_dir = os.path.abspath("time_gan_pt")
    if not os.path.exists(code_dir):
        raise ValueError(f"Code directory not found at {code_dir}.")

    # Data directory
    data_path = os.path.abspath("time_gan_pt/data")
    ...
    if args.is_train:
        if is_do_train:
            result_dict = do_timegan_trainer(model, train_data, train_time, args)
        else:
    ...
    if not is_do_train:
        ...
        generated_data = []
        generated_time = train_time
        for i in range(len(result_dict["embedder_list"])):
            # Generate fake data
            Z = torch.rand((len(train_time), args.max_seq_len, args.Z_dim))
            Z = Z.to(args.device)
            generated_data.append(
                ex_inference(
                    Z=Z,
                    T=train_time,
                    generator=result_dict["generator_list"][i],
                    supervisor=result_dict["supervisor_list"][i],
                    recovery=result_dict["recovery_list"][i],
                )
            )
    ...
    print(result_dict["meta_strategy"])
    meta_strategy = result_dict["meta_strategy"]
    ...
    new_feat_pred_perf_list = []
    for gen_data in generated_data:
        # gen_data = gen_data.cpu()
        new_feat_pred_perf = feature_prediction(
            (gen_data, generated_time), (test_data, test_time), feat_idx
        )
        new_feat_pred_perf_list.append(new_feat_pred_perf)
    print(
    ...
    new_step_ahead_pred_perf_list = []
    for gen_data in generated_data:
        # gen_data = gen_data.cpu()
        new_step_ahead_pred_perf = one_step_ahead_prediction(
            (gen_data, generated_time), (test_data, test_time)
        )
        new_step_ahead_pred_perf_list.append(new_step_ahead_pred_perf)
