...
from backup.curv_gan_pt import manifolds
from backup.curv_gan_pt.utils.data_utils import read_embeddings
from backup.curv_gan_pt import WrappedNormal
...
class LinearClassifier:
    def __init__(self, config):
        ...
        self.c = torch.Tensor([1.0]) if config.c == None else torch.tensor([config.c])
        self.linear = HypLinear(
            self.manifold, config.dims * 2, config.num_classes, self.c, 0.2, True
        )
        self.opt = torch.optim.Adam(
            self.linear.parameters(), lr=0.01, weight_decay=0.001
        )
...
class Generator(nn.Module):
    def __init__(self, config):
        ...
        self.c = (
            nn.Parameter(torch.Tensor([1.0])).to(self.device)
            if config.c == None
            else torch.tensor([config.c]).to(self.device)
        )
        self.wrapped_nd = WrappedNormal(
            torch.tensor([0.0]).to(self.device),
            torch.tensor([1.0]).to(self.device),
            manifolds.PoincareBallWrapped(c=self.c),
        )
        self.mlp = HypLinear(
            self.manifold,
            config.dims,
