...
class BFS_trees:
    ...
    def __init__(self, root_nodes, graph, batch_num=1, app=None, rcmd=None):
        ...
        if self.batch_num * self.step < len(root_nodes):
            self.roots_lst.append(
                root_nodes[self.batch_num * self.step : len(root_nodes)]
            )
        ...
        self.cache_filename = "./graph_gan_pt/cache/" + app + "/" + dataset + ".pkl"
    ...
    def get_tree(self, root):
        if self.index < 0 or self.out_of_bound(root, self.roots_lst[self.index]):
            for i, roots in enumerate(self.roots_lst):
                if not self.out_of_bound(root, roots):
                    self.index = i
                    break

            filename = self.cache_filename + "_" + str(self.index)
