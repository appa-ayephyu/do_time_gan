...
class NodeClassificationEval:
    ...
    def eval_node_classification(self):
        ...
        x_train, y_train = (
            self.emd[: int(0.9 * self.n_node), :],
            self.labels_matrix[: int(0.9 * self.n_node), :],
        )
        x_test, y_test = (
            self.emd[int(0.9 * self.n_node) :, :],
            self.labels_matrix[int(0.9 * self.n_node) :, :],
        ...
        results["acc"] = accuracy_score(
            mlb.fit_transform(y_test_coo), mlb.fit_transform(preds)
        )
        for average in averages:
            results[average] = f1_score(
                mlb.fit_transform(y_test_coo), mlb.fit_transform(preds), average=average
            )
