import graph_gan_pt.GraphGAN.config as config
from graph_gan_pt.evaluation import link_prediction as lp
from graph_gan_pt.evaluation import node_classification as nc
from graph_gan_pt.evaluation import recommendation as rcmd


def lp_train_info(
    item, embed_filename, test_filename, test_neg_filename, n_node, n_embed, rcmd=None
):
    lpe = lp.LinkPredictEval(
        embed_filename, test_filename, test_neg_filename, n_node, n_embed
    )

    if rcmd is None:
        result = lpe.eval_link_prediction(emd=item.embedding_matrix.detach().numpy())
    else:
        result = lpe.eval_rcmd_link_prediction(
            rcmd=rcmd, emd=item.embedding_matrix.detach().numpy()
        )
...
def eval_while_train(gGAN, item_name, epoch, save=False):
    ...
    if config.app == "link_prediction":
        lp_info = lp_train_info(
            item,
            config.emb_filenames[i],
...
def eval(gGAN, epoch):
    ...
    for i in range(len(config.modes)):
        if config.app == "link_prediction":
            ...
            results.append(
                head_info + "\t" + acc_info + macro_info + "lr: " + str(lr[i]) + "\n"
            )
            ...
            rcmde = rcmd.Recommendation(
                config.emb_filenames[i], gGAN.n_node, config.n_emb, rcmd=gGAN.rcmd
            )
            ...
            for K in config.rcmd_K:
                ...
                head_info, precision_info, recall_info = eval_info_rcmd(
                    rcmd_result, K, i
                )
                results.append(
                    head_info
