...
# import collections
from graph_gan_pt.GraphGAN.discriminator import Discriminator
from graph_gan_pt.GraphGAN.generator import Generator
import graph_gan_pt.GraphGAN.config as config
from graph_gan_pt import utils
from graph_gan_pt import rcmd_util

# from src.evaluation import link_prediction as lp
# from src.evaluation import node_classification as nc
from graph_gan_pt.GraphGAN.BFS_trees import BFS_trees
...
class graphGAN:
    def __init__(self):
        ...
        if config.app == "link_prediction":
            self.graph = utils.read_edges(
                train_filename=config.train_filename, test_filename=config.test_filename
            )
        ...
        self.discriminator = Discriminator(
            n_node=self.n_node, node_emd_init=node_embed_init_d
        )
        ...
        if config.app == "recommendation":
            ...
            self.BFS_trees = BFS_trees(
                self.root_nodes, self.graph, batch_num=config.cache_batch
            )
    ...
    def prepare_data_for_d(self):
        ...
        for i in tqdm.tqdm(self.root_nodes):
            if np.random.rand() < config.update_ratio:
                pos = self.graph[i]
                neg, _ = self.sample(
                    i, self.BFS_trees.get_tree(i), len(pos), for_d=True
                )
                if neg is None:
                    none_cnt += 1
        ...
        for i in tqdm.tqdm(self.root_nodes):
            if np.random.rand() < config.update_ratio:
                sample, paths_from_i = self.sample(
                    i, self.BFS_trees.get_tree(i), config.n_sample_gen, for_d=False
                )
    ...
    def get_node_pairs_from_path(path):
        ...
        for i in range(len(path)):
            center_node = path[i]
            for j in range(
                max(i - config.window_size, 0),
                min(i + config.window_size + 1, len(path)),
            ):
                if i == j:
                    continue
                node = path[j]
    ...
    def write_embeddings_to_file(self):
        ...
        for i in range(2):
            ...
            with open(config.emb_filenames[i], "w+") as f:
                lines = [
                    str(self.n_node) + "\t" + str(config.n_emb) + "\n"
                ] + embedding_str
                f.writelines(lines)
