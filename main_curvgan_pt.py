...
import numpy as np
from curv_gan_pt import optimizers
from sklearn.model_selection import train_test_split
from sklearn.metrics import roc_auc_score, average_precision_score, f1_score
from curv_gan_pt.models import Generator, Discriminator, LinearClassifier
from curv_gan_pt.utils.data_utils import load_data
from curv_gan_pt.utils.train_utils import get_dir_name, sigmoid
from GraphRicciCurvature.OllivierRicci import OllivierRicci

parser = argparse.ArgumentParser()
parser.add_argument("--config", type=str, default="cora-lp-best.yaml")
...
class Model:
    ...
    def __init__(self, config, data):
        self.config = config
        self.data = data
        self.graph, self.nodelist, self.edgelist = (
            data["train_graph"],
            data["train_nodes"],
            data["train_edges"],
        )
        ...
        # Optimizers
        if config.lr_reduce_freq is None:
            config.lr_reduce_freq = config.max_epochs
        self.optimizer_g = getattr(optimizers, config.optimizer)(
        ...
        self.lr_scheduler_g = torch.optim.lr_scheduler.StepLR(
            self.optimizer_g,
            step_size=int(config.lr_reduce_freq),
            gamma=float(config.gamma),
        )
        self.optimizer_d = getattr(optimizers, config.optimizer)(
    ...
    def train_gen(self):
        ...
        info = {"Training time": 0.0, "G_Loss": 0.0, "fake_loss": 0.0, "dt_loss": 0.0}
        train_time = 0
        ...
        for index in range(batch_count):
            ...
            for node_id in self.nodelist[
                index
                * self.config.g_batch_size : (index + 1)
                * self.config.g_batch_size
            ]:
            ...
            pos_node_embedding = self.generator.embedding.index_select(
                0, torch.tensor(pos_node_ids).to(self.config.device)
            )
            ...
            if self.config.lmda > 0:
                dt_loss = self.distortion(
                    pos_node_ids, pos_node_neighbor_ids, neighbor_fake_embedding
                )
            else:
                dt_loss = torch.tensor(0.0)
            self.optimizer_g.zero_grad()
            fake_loss = F.binary_cross_entropy(
                self.discriminator(pos_node_embedding, node_fake_embedding),
                torch.ones(len(pos_node_ids)).to(self.config.device)
                * self.config.label_smooth,
            )
    ...
    def train_dis(self):
        ...
        for index in range(batch_count):
            ...
            for node_id in self.nodelist[
                index
                * self.config.d_batch_size : (index + 1)
                * self.config.d_batch_size
            ]:
                for k in range(self.config.walk_num):
                    walk = self.random_walk(node_id)
                    for t in walk:
    ...
    def train(self):
        ...
        if self.config.save:
            if not self.config.save_dir:
                ...
                models_dir = os.path.join(
                    "./save", self.config.dataset + "_" + self.config.task, date
                )
        ...
        for epoch in range(n_epochs):
            ...
            for d_epoch in range(self.config.d_epochs):
                ...
                if self.config.task == "nc":
                    micro_f1, macro_f1 = self.eval_node_classification(
                        self.discriminator.embedding
                    )
                ...
                if self.config.task == "gr":
                    result = self.eval_graph_reconstruction(
                        self.discriminator.embedding
                    )
                    logging.info(result)
                    # if roc > best_metric_d[0]:         # Update best info
                    ...
                    cnt_d = 0
                train_info = dict(
                    zip(train_info, map(lambda x: round(x, 6), train_info.values()))
                )
        ...
        logging.info(
            "Training ends. Generator's best: {:.4f} & {:.4f}; Discriminator's best: {:.4f} & {:.4f}".format(
    ...
    def eval_link_prediction(self, embs):
        ...
        preds = list(pos_scores.cpu().data.numpy()) + list(
            neg_scores.cpu().data.numpy()
        )
...
if __name__ == "__main__":
    ...
    yml_path = "./curv_gan_pt/configs/{}".format(args.config)
    with open(yml_path, "r", encoding="utf-8") as f:
        config = yaml.safe_load(f)
    config = Config(
        config
    )  # convert yaml dict to python object for more convinent reference
