...
from graph_gan_pt.GraphGAN import graphGAN, config

from graph_gan_pt.evaluation import evaluation
    ...
    for d_epoch in range(config.n_epochs_dis):
        ...
        for i, start in enumerate(start_list):
            ...
            loss = discriminator.loss(
                node_id=np.array(center_nodes[start:end]),
    ...
    for g_epoch in range(config.n_epochs_gen):
        ...
        for i, start in enumerate(start_list):
            ...
            score = generator.score(
                node_id=np.array(node_1[start:end]),
                node_neighbor_id=np.array(node_2[start:end]),
            )
            prob = discriminator(score)
            loss = generator.loss(prob=prob, reward=reward[start:end])
