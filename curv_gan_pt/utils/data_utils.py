...
def read_graph(config):
    if config.dataset in ["cora", "citeseer", "webkb"]:
        idx_features_labels = np.genfromtxt(
            "{}/{}/{}.content".format(data_path, config.dataset, config.dataset),
            dtype=np.dtype(str),
        )
        ...
        edges_unordered = np.genfromtxt(
            "{}/{}/{}.cites".format(data_path, config.dataset, config.dataset),
            dtype=np.dtype(str),
        )
        edges = np.array(
        ...
        edges = np.array(
            pd.read_csv(
                "{}/{}/adjacency.csv".format(data_path, config.dataset), header=None
            )
        )
        labels = np.array(
            pd.read_csv(
                "{}/{}/labels.csv".format(data_path, config.dataset), header=None
            )
        ).T.squeeze()
        classes = sorted(list(set(labels)))
        print("Num of classes: {}".format(len(classes)))
        ...
        edges = (
            torch.load(
                "{}/{}/{}_edge_index.pt".format(
                    data_path, config.dataset, config.dataset
                )
            )
            .data.numpy()
            .T
        ...
        try:
            ...
            labels = None
        features = torch.load(
            "{}/{}/{}_features.pt".format(data_path, config.dataset, config.dataset)
        )
...
def load_data(config):
    ...
    if config.task in ["gr"]:
        ...
        data = {
            ...
            "test_nodes": set(
                random.sample(
                    list(range(config.num_nodes)), math.ceil(0.1 * config.num_nodes)
                )
            ),
            "train_graph": nx.Graph(train_edges.tolist()),
