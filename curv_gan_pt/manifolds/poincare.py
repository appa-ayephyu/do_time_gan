...
from curv_gan_pt.manifolds.base import Manifold
from curv_gan_pt.utils.math_utils import artanh, tanh
...
class PoincareBallWrapped(PoincareBallParent):
    ...
    def logdetexp(self, x, y, is_vector=False, keepdim=False):
        ...
        return (self.dim - 1) * (
            torch.sinh(self.c.sqrt() * d) / self.c.sqrt() / d
        ).log()
...
class PoincareBall(Manifold):
    ...
    def sqdist(self, p1, p2, c):
        ...
        dist_c = artanh(
            sqrt_c
            * (self.mobius_add(-p1, p2, c, dim=-1) + self.min_norm).norm(
